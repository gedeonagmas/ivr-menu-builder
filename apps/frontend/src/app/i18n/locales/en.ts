export const en = {
  header: {
    folderName: 'Folder Name',
    projectSelection: {
      rename: 'Rename',
      duplicateToDrafts: 'Duplicate to Drafts',
    },
    controls: {
      export: 'Export',
      import: 'Import',
      saveAsImage: 'Save as image',
      archive: 'Archive',
    },
  },
  palette: {
    templates: 'Templates',
    nodesLibrary: 'Nodes Library',
    helpSupport: 'Help & Support',
  },
  node: {
    action: {
      label: 'Action',
      description: 'Perform actions based on triggers',
    },
    conditional: {
      label: 'Conditional',
      description: 'Branch the workflow',
    },
    decision: {
      label: 'Decision',
      description: 'Route the workflow',
    },
    delay: {
      label: 'Delay',
      description: 'Pause the workflow',
    },
    notification: {
      label: 'Notification',
      description: 'Send alerts or notifications',
    },
    trigger: {
      label: 'Trigger',
      description: 'Initiate workflows',
    },
    answerCall: {
      label: 'Answer Call',
      description: 'Answer incoming phone call',
    },
    hangUpCall: {
      label: 'Hang Up Call',
      description: 'End the call',
    },
    playAudioTTS: {
      label: 'Play Audio or TTS',
      description: 'Play audio file or text-to-speech message',
    },
    sendSMS: {
      label: 'Send SMS',
      description: 'Send SMS message',
    },
    gatherInput: {
      label: 'Gather Input',
      description: 'Collect input with dynamic menu options',
    },
    forwardToPhone: {
      label: 'Forward to Phone',
      description: 'Forward call to phone number with status connectors',
    },
    startCallRecording: {
      label: 'Start Call Recording',
      description: 'Start recording the call',
    },
    stopCallRecording: {
      label: 'Stop Call Recording',
      description: 'Stop recording the call',
    },
    voicemailRecording: {
      label: 'Voicemail Recording',
      description: 'Record voicemail message',
    },
    request: {
      label: 'Request',
      description: 'Make HTTP request with condition connectors',
    },
    conditions: {
      label: 'Conditions',
      description: 'Evaluate conditions with dynamic branches',
    },
    executeSWML: {
      label: 'Execute SWML',
      description: 'Execute SWML script with condition connectors',
    },
    setVariables: {
      label: 'Set Variables',
      description: 'Set workflow variables',
    },
    unsetVariables: {
      label: 'Unset Variables',
      description: 'Unset workflow variables',
    },
  },
  propertiesBar: {
    label: 'Properties',
    deleteNode: 'Delete node',
    deleteEdge: 'Delete edge',
  },
  deleteConfirmation: {
    text: "You're about to <b>permanently delete</b> {{selected}} {{parts}}. Please, confirm to proceed.",
    cancel: 'Cancel',
    delete: 'Delete',
    node: 'node',
    nodes: 'nodes',
    edge: 'edge',
    edges: 'edges',
    andConnected: 'and connected',
    selected: 'selected',
    selectedPlural: 'selected',
    deleteSelection: 'Delete Selection?',
    dontShowMeThisAgain: 'Don’t show me this again',
  },
  decisionBranches: {
    branch: 'Branch #{{index}}',
    branches: 'Branches',
    addBranch: 'Add branch',
    singleCondition: '1 condition',
    manyConditions: '{{count}} conditions',
    noConditions: 'No conditions yet',
  },
  conditions: {
    title: 'Conditional Editor',
    subtitle: 'Define Your Conditional Rules',
    cancel: 'Cancel',
    confirm: 'Confirm',
    totalNumber: 'total: {{count}} conditions',
    totalNumber_one: 'total: 1 condition',
    dependencies: 'Dependencies',
    compare: {
      or: 'or',
      and: 'and',
      isEqual: 'is equal to',
      isNotEqual: 'is not equal to',
      isGreaterThan: 'is greater than',
      isLessThan: 'is less than',
      isLessThanOrEqual: 'is less than or equal to',
      isGreaterThanOrEqual: 'is greater than or equal to',
      isContaining: 'contains',
      isNotContaining: 'does not contain',
    },
  },
  loader: {
    text: 'Loading...',
  },
  noAccess: {
    header: 'Unlock Full Product Access',
    title: 'Reach out to learn more',
    subtitle: 'Connect with us for full access details.',
  },
  templateSelector: {
    title: 'Select a template',
    description: 'Get started quickly with pre-designed templates<br/>for your Workflow Editor',
    emptyCanvas: 'Empty Canvas',
  },
  tooltips: {
    exportDiagram: 'Export File',
    readOnlyMode: 'Toggle Read Only Mode',
    layout: 'Layout Diagram',
    open: 'Open',
    close: 'Close',
    palette: 'Palette',
    propSidebar: 'Properties Sidebar',
    remove: 'Remove',
    cantRemoveOnlyOption: "You can't remove the only option",
    undo: 'Undo',
    redo: 'Redo',
    addOption: 'Add Option',
    menu: 'Menu',
    pickTheProject: 'Pick the project',
    openPalette: 'Open palette',
    closePalette: 'Close palette',
    importDiagram: 'Import File',
    save: 'Save',
    simulate: 'Simulate IVR Flow',
    pause: 'Pause Simulation',
    resume: 'Resume Simulation',
    deploy: 'Deploy to Twilio',
    changeLanguage: 'Change Language',
  },
  snackbar: {
    saveDiagramSuccess: 'Saving diagram successfully',
    noDiagramToLoad: 'No diagram to load!',
    loadDiagramError: 'An error occurred while loading diagram',
    saveDiagramError: 'An error occurred while saving diagram',
    restoreDiagramSuccess: 'Locally saved data has been restored',
    restoreDiagramError: 'Could not load locally saved data',
    aiConnectionError: 'Error in connection to AI server',
    wrongDiagramFormat: 'Wrong diagram format',
  },
} as const;
